...
// TODO: create a simple redux form
...
const App: React.FC = () => {
  const [displayUserInfo, setDisplayUserInfo] = useState({username:"", password:"", name:"", emailAddress:"" });
  // TODO: add useEffect to call simple API when React DOM mounts ---(DONE)
  useEffect(() => {
    getUser().then(
      res=>{
        setDisplayUserInfo({...displayUserInfo, username:res.username, name:res.name, emailAddress:res.email});
      }, 
      err=>{
